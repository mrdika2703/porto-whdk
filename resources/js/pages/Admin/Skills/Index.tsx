...
export default function Index({ skills, hasProfile, flash }: IndexProps) {
    useEffect(() => {
        if (flash.success) {
            Swal.fire({
                icon: 'success',
    ...
    const handleDelete = (id: number) => {
        Swal.fire({
            title: 'Hapus Data?',
            text: 'Keahlian ini akan dihapus permanen!',
            icon: 'warning',
    ...
    // Fungsi warna dinamis untuk Level
    const getLevelColor = (level: string | null) => {
        switch (level) {
            ...
            case 'Beginner':
                return 'bg-tmuted/10 text-tmuted border-tmuted/20';
            default:
                return 'bg-main/20 text-tmuted border-bmain/20';
        }
    };
            ...
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-tmuted">
                        Kelola daftar kemampuan teknis (Hard Skill) dan
                        interpersonal (Soft Skill) Anda.
                    </p>
                </div>
                ...
                {hasProfile ? (
                    ...
                    >
                        <i className="fa-solid fa-plus"></i> Tambah Keahlian
                    </Link>
                ) : (
            ...
            <div className="overflow-x-auto rounded-2xl border border-bmain/20 bg-bcard p-6 shadow-sm">
                <table className="w-full text-left text-sm text-htext dark:text-tmain">
                    <thead className="border-b border-bmain/20 text-xs text-tmuted uppercase">
                        <tr>
                            <th className="w-16 px-4 py-3">Ikon</th>
                    ...
                    <tbody>
                        {skills.length > 0 ? (
                            skills.map((item) => (
                                ...
                                >
                                    <td className="px-4 py-3 text-center text-xl text-accent">
                                        {item.icon ? (
                                            <i className={item.icon}></i>
                                        ) : (
                                            <span className="text-xs text-tmuted/50">
                                                -
                                            </span>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-4 py-3">
                                        <span
                                            className={`rounded-md border px-2 py-1 text-xs ${item.category === 'Hard Skill' ? 'border-orange-500/20 bg-orange-500/10 text-orange-500' : 'border-purple-500/20 bg-purple-500/10 text-purple-500'}`}
                                        >
                                            {item.category}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3">
                                        {item.level ? (
                                            <span
                                                className={`rounded-md border px-2 py-1 text-xs font-semibold ${getLevelColor(item.level)}`}
                                            >
                                                {item.level}
                                            </span>
                                        ) : (
                                            <span className="text-xs text-tmuted italic">
                                                Belum diukur
                                            </span>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex justify-end gap-2">
                                            <Link
                                                href={`/admin/skills/${item.id}/edit`}
                            ...
                            <tr>
                                ...
                                >
                                    <div className="flex flex-col items-center justify-center gap-2">
                                        ...
                                        <p>Belum ada data keahlian (skills).</p>
                                    </div>
                                </td>
                            </tr>
                        )}
